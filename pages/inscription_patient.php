...
if ($table_medecins_exists) {
    ...
    if (!empty($search_med)) {
        ...
        $search_conditions_group[] = "LOWER(nom) LIKE LOWER(:search_med_nom)";
        $params_for_query[':search_med_nom'] = $search_value_param;

        $search_conditions_group[] = "LOWER(prenom) LIKE LOWER(:search_med_prenom)";
        $params_for_query[':search_med_prenom'] = $search_value_param;

        $search_conditions_group[] = "LOWER(email) LIKE LOWER(:search_med_email)";
        $params_for_query[':search_med_email'] = $search_value_param;

        $search_conditions_group[] = "LOWER(specialite) LIKE LOWER(:search_med_spec_text)";
        $params_for_query[':search_med_spec_text'] = $search_value_param;
...
$nb_com_att_nav_gest_med = 0;
$nb_med_att_nav_gest_med = 0; 

if (isset($pdo)) { 
    try {
        if ($pdo->query("SHOW TABLES LIKE 'commentaires'")->rowCount() > 0) {
            $nb_com_att_nav_gest_med = (int)$pdo->query("SELECT COUNT(*) FROM commentaires WHERE statut = 'en attente'")->fetchColumn();
        }
        if ($table_medecins_exists) { 
            $nb_med_att_nav_gest_med = (int)$pdo->query("SELECT COUNT(*) FROM medecins WHERE valide = 0")->fetchColumn();
        }
    } catch (PDOException $e_nav) {
        error_log("Erreur comptage badges nav (gestion_medecins): " . $e_nav->getMessage());
    }
}
?>
<!DOCTYPE html>
    ...
    <div class="container">
        ...
        <nav class="main-navigation admin-navigation" id="main-nav">
            <ul>
                ...
                <li><a href="gestion_commentaires.php" class="nav-link">Commentaires <?php if(isset($nb_com_att_nav_gest_med) && $nb_com_att_nav_gest_med > 0): ?><span class="badge-notification"><?= $nb_com_att_nav_gest_med ?></span><?php endif; ?></a></li>
