...
$default_redirect_page_val_med = '../admin/gestion_medecins.php'; 
$return_url_val_med = trim($_GET['return_url'] ?? '');
...
if (!empty($return_url_val_med)) {
    ...
    if (isset($parsed_return_url['path'])) {
        foreach ($allowed_paths as $allowed_path) {
            if (strpos(basename($parsed_return_url['path']), $allowed_path) !== false) {
                $path_is_allowed = true; break;
            }
        }
    }
    if ($path_is_allowed) {
        $redirect_page_val_med = basename($parsed_return_url['path']); 
        if (isset($parsed_return_url['query'])) {
            $redirect_page_val_med .= '?' . $parsed_return_url['query'];
        }
    }
}
// Pour la redirection, si le script est dans php/ et la page admin dans admin/
if (strpos($redirect_page_val_med, "admin/") !== 0 && $redirect_page_val_med !== $default_redirect_page_val_med) {
    $redirect_page_val_med = "../admin/" . $redirect_page_val_med;
}
...
$medecin_id_a_valider = (int)$_GET['id'];
    ...
    if ($stmt_update_med_status->execute([$medecin_id_a_valider])) {
        if ($stmt_update_med_status->rowCount() > 0) {
            ...
            if (function_exists('envoyer_email') && function_exists('get_email_html_layout') && !empty($medecin_data_to_validate['email'])) {
                ...
                $base_path_email_val = dirname(dirname($_SERVER['PHP_SELF']));
                if (basename($base_path_email_val) === 'php') $base_path_email_val = dirname($base_path_email_val);
                ...
                $contenu_principal_email_med = "
                    <p>Bonjour " . $nom_medecin_email_val . ",</p>
                    <p>Bonne nouvelle ! Votre demande d'inscription sur la plateforme SANTE TV a été examinée et <strong>approuvée</strong> par notre équipe d'administration.</p>
                    <p>Votre compte médecin est maintenant actif. Vous pouvez dès à présent vous connecter à votre espace personnel pour :</p>
                    <ul style='list-style-type: disc; margin-left: 20px; padding-left: 5px;'>
                        <li>Compléter et gérer votre profil professionnel public.</li>
                        <li>Définir vos plages de disponibilité pour les consultations.</li>
                        <li>Commencer à recevoir et gérer les demandes de rendez-vous de patients.</li>
                    </ul>
                    <div class='button-container'>
                        <a href='" . $lien_connexion_email_val . "' class='button'>Accéder à Mon Espace Médecin</a>
                    </div>
                    <p>Si vous avez des questions ou besoin d'assistance pour démarrer, n'hésitez pas à consulter notre section FAQ ou à contacter notre support.</p>
                    <p>Nous sommes ravis de vous accueillir au sein de la communauté SANTE TV !</p>
                ...
                envoyer_email(
                    $medecin_data_to_validate['email'], 
                    $nom_medecin_email_val, 
                    $sujet_email_validation_med, 
                    $corps_html_email_validation_med
                );
            }
        } else {
